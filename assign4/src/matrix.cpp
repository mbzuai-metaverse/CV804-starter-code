...
Matrix::Matrix(const Vec3d& v): rows_(3), cols_(1) {
    allocSpace();
    p[0][0] = v[0];
    p[1][0] = v[1];
    p[2][0] = v[2];
...
Matrix operator*(const Matrix& m, const Vec3d& v)
{
    Matrix temp(v);
    return m * temp;
}
...
Matrix::operator Vec3d() const {
    assert (rows_ == 3 && cols_ == 1);
    return Vec3d(this->p[0][0], this->p[1][0], this->p[2][0]);
}
